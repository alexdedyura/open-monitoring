<script>
  ...
  // Both formatters read `lang` / call t(), so they are only reactive because
  // every call site is a template expression — never hoist them into a const.
  function fmtDate(ms) {
    return new Date(ms).toLocaleString(lang.code, {
  ...
  function fmtDur(rec) {
    if (!rec.endedAt) return t('sess.dur.recording')
    const sec = Math.round((rec.endedAt - rec.startedAt) / 1000)
    const h = Math.floor(sec / 3600)
    const m = Math.floor(sec / 60) % 60
    return h ? t('sess.dur.hourMin', {h, m}) : t('sess.dur.minSec', {m, s: sec % 60})
  }
...
{:else}
  <div class="space-y-3 p-4">
    <div class="flex items-baseline justify-between">
      <h2 class="font-mono text-[11px] uppercase tracking-[0.14em] text-mut">{t('sess.title')}</h2>
      {#if exportedTo}
        <span class="font-mono text-[11px] text-ink2">{t('sess.savedTo', {path: exportedTo})}</span>
      {/if}
    </div>
          ...
          <div class="flex items-center gap-4 px-4 py-3 hover:bg-card2">
            <button class="min-w-0 grow text-left" onclick={() => (open = sess)}>
              ...
              <div class="mt-0.5 font-mono text-[11px] text-mut">
                {fmtDate(sess.startedAt)} · {fmtDur(sess)} · {t('sess.samples', {n: sess.samples, count: fmtNum(sess.samples)})} · {t('sess.interval', {ms: sess.intervalMs})}
              </div>
            </button>
            <button
            ...
            >
              {exporting === sess.id ? t('sess.rendering') : 'PNG'}
            </button>
            <button
              ...
              onclick={() => del(sess.id)}
            >
              {confirmDel === sess.id ? t('sess.deleteConfirm') : t('sess.delete')}
            </button>
          </div>
        {/each}
      </div>
